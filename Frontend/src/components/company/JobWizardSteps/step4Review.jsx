  ...
  return (
    <div className="w-full max-w-4xl mx-auto rounded-2xl border border-line bg-raised p-6 sm:p-8 shadow-2xl backdrop-blur-md">
      ...
      <div className="mb-6">
        ...
        <h2 className="text-xl font-bold tracking-tight text-content mt-4">Review</h2>
      ...
      <div className="rounded-xl border border-line bg-surface/50 p-5 space-y-3">
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="font-bold text-content">{data.department || '—'}</span>
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="text-subtle">Work Type</span>
          <span className="font-bold text-content">{data.workEnvironment || '—'}</span>
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="text-subtle">Duration</span>
          <span className="font-bold text-content">{data.durationValue ? `${data.durationValue} ${data.durationUnit}` : '—'}</span>
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="text-subtle">Pay Range</span>
          <span className="font-bold text-accent">{data.payMin && data.payMax ? `$${data.payMin} - $${data.payMax} / mo` : '—'}</span>
        </div>
      </div>
        ...
        <div className="mt-4 rounded-xl border border-line bg-surface/30 p-4">
          ...
          <ul className="mt-2 space-y-1">
            {data.skills.map((skill, idx) => (
              <li key={idx} className="text-xs text-subtle flex items-start gap-2">
