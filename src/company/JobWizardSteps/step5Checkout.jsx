...
export default function Step5Checkout({ data, onChange, onSubmit, onBack, loading, error, showToast }) {
  ...
  const errClass = (field) =>
    touched[field] && errors[field]
      ...
      : 'border-white/5 focus:border-emerald-500/40';
  ...
  return (
    <div className="w-full max-w-2xl mx-auto rounded-2xl border border-white/5 bg-[#111B34]/40 p-6 sm:p-8 shadow-2xl backdrop-blur-md">
      ...
      <div className="text-center pb-6 border-b border-white/5 mb-6">
        ...
        <p className="text-xs text-gray-400 mt-1">Review your details and select a visibility plan before publishing.</p>
      </div>
      ...
      <div className="space-y-5">
        ...
        <div className="border-t border-white/5 pt-4 space-y-3.5">
          ...
          <div>
            <input
              ...
              className={`w-full px-4 py-2.5 rounded-xl border bg-[#070B19]/60 text-xs text-white focus:outline-none transition ${errClass('cardName')}`}
            />
          ...
          <div>
            <input
              ...
              maxLength={19}
              className={`w-full px-4 py-2.5 rounded-xl border bg-[#070B19]/60 text-xs text-white tracking-wider focus:outline-none transition ${errClass('cardNumber')}`}
            />
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl border bg-[#070B19]/60 focus-within:border-emerald-500/40 transition ${errClass('expiry')}`}>
                <input
                  ref={monthInputRef}
                  type="text"
                  inputMode="numeric"
                  ...
                  placeholder="MM"
                  maxLength={2}
                  className="w-6 bg-transparent text-xs text-white text-center focus:outline-none"
                />
            ...
            <div>
              <input
                ...
                placeholder="CVC"
                maxLength={4}
                className={`w-full px-4 py-2.5 rounded-xl border bg-[#070B19]/60 text-xs text-white focus:outline-none transition text-center ${errClass('cvc')}`}
              />
        ...
        <div className="pt-4 border-t border-white/5 flex items-center justify-between">
          <div>
            ...
            <span className="text-2xl font-black text-emerald-400">${selectedPlan.price}.00</span>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={onBack} disabled={loading} className="px-4 py-2.5 rounded-xl text-xs font-semibold border border-white/10 text-gray-400 hover:bg-white/5 transition disabled:opacity-40">
              Back
            </button>
            <button
